...
def upload_to_s3(file, filename):
    ...
    try:
        ...
        # Read the file content using .read() and upload it to S3
        file_content = file.read()
        with fs.open(s3_path, 'wb') as f:
            f.write(file_content)  # Writing the file to the S3 bucket
...
"""
def process_single_pdf_from_s3(file_key, required_skills, good_to_have_skills, processed_hashes):
    #Processes a single resume PDF from S3.
    file_hash = file_key  # S3 object key as the unique hash for the file
    if file_hash in processed_hashes:
        return None

    # Fetch the file directly from S3
    file = fs.open(f"{file_key}", 'rb')

    # Extract text using AWS Textract
    text = load_pdf_with_textract(file,file_key)
...
def main():
    ...
    st.subheader("Process resumes directly from S3 bucket")
    ...
    # List files in the 'resumes' folder inside the S3 bucket
    folder_path = 'resumes/'  # Folder name in S3
    files_in_s3 = fs.ls(f"{S3_BUCKET_NAME}/{folder_path}")
    #pdf_files = [f for f in files_in_s3 if f.endswith(('pdf', 'png', 'jpg', 'jpeg', 'docx'))]
    pdf_files = [f.split('/')[-1] for f in files_in_s3 if f.endswith(('pdf', 'png', 'jpg', 'jpeg', 'docx'))]

    # Display the files and provide an option to process them
    st.write("Files available in 'resumes' folder:")
    st.write(pdf_files)

    # Automatically process all files in the 'resumes' folder
    if pdf_files:
        if st.button("Process all files"):
            ...
            for file_name in pdf_files:
                ...
                skills = extract_skills(text)
                matched_required, matched_good_to_have, match_percentage = evaluate_skills(skills, required_skills, good_to_have_skills)

                all_candidates.append({
                    "Filename": file_name,
                    "Matched Skills": ", ".join(matched_required),
                    "Good-to-Have Skills": ", ".join(matched_good_to_have),
        ...
        st.write("No files found in the 'resumes' folder.")
